                    ...
                    <div class="carousel-track">
                        ${images.map((img, idx) => `
                            <div class="carousel-slide ${idx === 0 ? 'active' : ''}" data-index="${idx}">
                                <img src="images/${img}" alt="${circle.name}" loading="lazy" onerror="this.parentElement.style.display='none'">
                            </div>
                        `).join('')}
                    </div>
                    ${images.length > 1 ? `
    ...
    practiceImages.forEach(imgPath => {
        ...
        html += `
            <div class="gallery-item">
                <img src="images/${imgPath}" alt="协商民主实践活动" loading="lazy" onerror="this.style.display='none'">
...
function renderPlatformGallery() {
    ...
    studios.forEach(studio => {
        if (studio.image) {
            ...
            html += `
                <div class="gallery-item">
                    <img src="images/${studio.image}" alt="${studio.name}" loading="lazy" onerror="this.style.display='none'">
